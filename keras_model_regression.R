...
spec <- feature_spec(dataset = train, price ~ .)
...
inputs <- layer_input_from_dataset(train |> select(-price))

outputs <- inputs |> layer_dense_features(dense_features(spec)) |> 
    layer_dense(units = 2096, activation = 'relu') |> 
    layer_dense(units = 1048, activation = 'relu') |> 
